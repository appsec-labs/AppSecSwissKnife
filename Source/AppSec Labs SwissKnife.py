...
def RegFilter(output,filter):
    ...
    for patternNode in filter.find('FilterExpressions').find('Expression').find('RegEx').iter('Pattern'):
        ...
        for j in range(0,len(findall)):
            if(not re.search(re.compile(pattern),output)==None):
                    ...
                    #print("group" + re.search(re.compile(pattern),output).group(0));
                    results = re.search(re.compile(pattern),output).group(0);
                    #print("results" + results);
                    if(not resultEdit.replace("{"+str(0)+ "}",result) +"\n" in result):
                            result += resultEdit.replace("{"+str(0)+ "}",results);
                            result+="\n"
    ...
    #print("printing results of reg:"+ result);            
    return result;
...
banner = "\n     AppSec Labs Swiss Knife Network Scanner\n\n       Read more at: https://appsec-labs.com/tools\n       Author:  Gilad Ofir  -\n                Information \\ Application Security Consultant "
usage='usage: AppSecSwissKnife.exe hostname [-f hostfile]  [-p port] [-o outputdir]\nAppSecSwissKnife.exe or -h for help'
...
def main():
        ...
        print("---------------------------------------\nDone running AppSec Lab Swiss Knife....");
        try:
...
def modCmdExec(cmdoption,source,dest,modExe,modName,xmlModeName,strToolName,isFile,port):
        try: 
            ...
            error = "";
                #print ("AppSec Swiss Knife error:");
